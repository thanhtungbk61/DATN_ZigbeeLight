...
volatile uint16_t sensorValue = 400;
uint8_t pingLen =8;
...
void emberAfMainInitCallback(void)
{
	...
	emberEventControlSetDelayMS(commissioningLedEventControl,7 * MY_DELAY_IN_MS);
...
//	  GPIO_PinOutToggle(LED_PORT,LED_PIN);
	  emberEventControlSetDelayMS(commissioningLedEventControl, LED_DELAY_MS);
//    EmberStatus status = emberAfPluginNetworkSteeringStart();
//    emberAfCorePrintln("%p network %p: 0x%X", "Join", "start", status);
  }
}
...
emberAfCorePrintln("report--sensor:%d",sensorValue);
