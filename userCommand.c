...
uint8_t reportSensor(uint16_t value)
{
	uint8_t valueWrite[2];
	valueWrite[0] = value>>8;
	valueWrite[1] = (uint8_t) value&0x00ff;
	emberAfWriteServerAttribute(1,ZCL_ILLUM_MEASUREMENT_CLUSTER_ID,ZCL_ILLUM_MEASURED_VALUE_ATTRIBUTE_ID,valueWrite,0x21);
	uint8_t status=1;
//    if (emberAfReadServerAttribute(1,
...
//    {
	...
	buf[1]=0;   // attribute
	buf[2]=0;   // status
	buf[3]=0x21;  // type value
	buf[4]=value>>8;  //  value
	buf[5]=(uint8_t) value&0x00ff;
	emberAfCorePrintln("sensorValue[0]:%d----sensorValue[1]:%d",buf[4],buf[5]);
